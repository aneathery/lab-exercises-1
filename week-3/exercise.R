...
iris_df$petal_length_4 <- iris_df$Petal.Length > 4.0

# Find the species type of the flower that has the longest Sepal length

iris_df[iris_df$Petal.Length == max(iris_df$Petal.Length), "Species"]

# Select rows where species type is 'setosa' and Sepal.Width > 3.0
#iris_df[iris$Species == 'setosa', "Sepal.Width" > 3.0, ]
